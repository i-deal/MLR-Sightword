...
# 10 rand 2 digit numbers each digit repr once in each pos.
# padded cifar10, 2d retina, sight word latent, 
def dataset_builder(data_set_flag, bs, return_class = False):
    if data_set_flag == 'sight_word_single':
        class translate_across_retina:
    ...
    if data_set_flag == 'sightwords': 
        train_dataset = torch.load('data/sightwords_MNIST/training.pt')
        test_dataset = torch.load('data/sightwords_MNIST/test.pt')
        # iterate through train_dataset

        train_loader = torch.utils.data.DataLoader(train_dataset, shuffle = True, batch_size=bs, drop_last=True)

        test_loader = torch.utils.data.DataLoader(test_dataset, shuffle = True, batch_size=bs, drop_last=True)
...
class VAE_CNN(nn.Module):
    def __init__(self, x_dim, h_dim1, h_dim2, z_dim, l_dim):
        ...
        self.fc37 = nn.Linear(h_dim2, 2*z_dim) # sightword
        self.fc38 = nn.Linear(h_dim2, 2*z_dim)
        # decoder part
        self.fc4s = nn.Linear(z_dim, h_dim2)  # shape
        self.fc4c = nn.Linear(z_dim, h_dim2)  # color
        self.fc4l = nn.Linear(z_dim, l_dim)  # location
        self.fc4w = nn.Linear(2*z_dim, h_dim2) # sightword
    ...
    def decoder_sightword(self, z_sightword):
        h = F.relu(self.fc4w(z_sightword))
        h = F.relu(self.fc5(h)).view(-1, 16, int(imgsize / 4), int(imgsize / 4))
        h = self.relu(self.bn5(self.conv5(h)))
        h = self.relu(self.bn6(self.conv6(h)))
        h = self.relu(self.bn7(self.conv7(h)))
    ...
    def forward(self, x, whichdecode='noskip', keepgrad=[]):
        ...
        mu_shape, log_var_shape, mu_color, log_var_color, mu_sightword, log_var_sightword, mu_location, log_var_location, hskip = self.encoder(x, l)
        ...
        if ('sightword' in keepgrad):
            z_sightword = self.sampling(mu_sightword, log_var_sightword)
        else:
            z_sightword = self.sampling(mu_sightword, log_var_sightword).detach()
        ...
        elif (whichdecode == 'sightword'):
            output = self.decoder_sightword(z_sightword)
...
size1 = 2*imgsize # temporary fix to make adjustments to the loss functions faster
...
def loss_function_shape(recon_x, x, mu, log_var):
    ...
    #save_image(grayrecon.clone()[0],"grayloss_sample.png")
    #grayrecon = grayrecon.cuda()
...
def progress_out(data, epoch, count, skip = False, sightword = False):
    sample_size = 25
    vae.eval()
    ...
    if skip:
        ...
        with torch.no_grad():
            ...
            reconc = torch.cat((reconc.view(sample_size, 3, imgsize, -1).cuda(),line1),dim = 3).cuda()
            recons = torch.cat((recons.view(sample_size, 3, imgsize, -1).cuda(),line1),dim = 3).cuda()
            recond = torch.cat((recond.view(sample_size, 3, imgsize, -1).cuda(),line1),dim = 3).cuda()
            sample = torch.cat((sample.view(sample_size, 3, imgsize, -1).cuda(),line1),dim = 3).cuda()
            reconds = torch.cat((reconds.view(sample_size, 3, imgsize, -1).cuda(),line1.cuda()),dim = 3).cuda()
            utils.save_image(
        ...
        with torch.no_grad():
            shape_color_dim = retina_size + 2
            reconds, mu_color, log_var_color, mu_shape, log_var_shape, mu_location, log_var_location = vae(sample, 'skip_cropped') #digit from skip
            reconsw, mu_color, log_var_color, mu_shape, log_var_shape, mu_location, log_var_location = vae(sample, 'sightword') #digit

            reconsw = torch.cat((reconsw.view(sample_size, 3, imgsize, -1).cuda(),line1),dim = 3).cuda()
            sample = torch.cat((sample.view(sample_size, 3, imgsize, -1).cuda(),line1),dim = 3).cuda()
            reconds = torch.cat((reconds.view(sample_size, 3, imgsize, -1).cuda(),line1.cuda()),dim = 3).cuda()
            utils.save_image(
            torch.cat([sample.view(sample_size, 3, imgsize, shape_color_dim).cuda(), reconds.view(sample_size, 3, imgsize, shape_color_dim).cuda(), reconsw.view(sample_size, 3, imgsize, shape_color_dim).cuda()], 0),
            f'sample_{vae_type_flag}_{data_set_flag}/{str(epoch + 1).zfill(5)}_{str(count).zfill(5)}.png',
            nrow=sample_size, normalize=False, range=(-1, 1),)
...
#single, skip, sightword train functions
def train(epoch, whichdecode, train_loader_noSkip, train_loader_skip, test_loader_noSkip):
    global numcolors
    ...
    m = 4 # number of seperate training decoders used
    ...
    for i in loader:
        ...
        if (whichdecode == 'single'):  # if yes, randomly alternate between using and ignoring the skip connections
            if count % m == 0:  # one of out 3 times, let's use the skip connection
                loss = loss_function_shape(recon_batch, data[0], mu_shape, log_var_shape)  # change the order, was color and shape
                loss.backward()

            elif count% m == 1:
                loss = loss_function_color(recon_batch, data[0], mu_color, log_var_color)
                loss.backward()
                ...
                loss = loss_function_crop(recon_batch, data[0], mu_shape, log_var_shape, mu_color, log_var_color)
                loss.backward()
