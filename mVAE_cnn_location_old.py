...
data_set_flag = 'padded_mnist' # mnist, cifar10, padded_mnist
...
if data_set_flag == 'padded_mnist':
    ...
    class translate_to_right:
        def __init__(self, max_width):
            ...
            self.box = transforms.ToPILImage()(torch.full((3,28,28), 0.05))
        def __call__(self, img):
            ...
            pos = torch.zeros((28))
            pos[(7*padding_left)//25] = 1
            return ImageOps.expand(img, padding), ImageOps.expand(self.box, padding), pos
    ...
    class PadAndPosition:
        ...
        def __call__(self, img):
            new_img, location, position = self.transform(img)
            return transforms.ToTensor()(new_img), transforms.ToTensor()(location), transforms.ToTensor()(img), transforms.ToTensor()(img) + position
    ...
    img, pos = train_dataset_right[0]
    print("X position:", pos)
    
    # Load MNIST datasets
    train_dataset_right = datasets.MNIST( #loads an MNIST dataset where every digit is translated to the right half of the retina space
        root='./data',
        train=True,
        download=True,
        transform=transforms.Compose([
            transforms.Resize((28, 28)),
            Colorize_func,
            translate_to_right(retina_size),
            transforms.ToTensor()
        ])
    )

    train_dataset_left = datasets.MNIST( #loads an MNIST dataset where every digit is translated to the left half of the retina space
        root='./data',
        train=True,
        download=True,
        transform=transforms.Compose([
...
#set default flags for layer sizes etc...
#pad transform for placing an mnist character within the retina space
...
class VAE_CNN(nn.Module):
    def __init__(self, x_dim, h_dim1, h_dim2, z_dim):
        ...
        self.fc2 = nn.Linear(int(retina_size / 4) * int(imgsize / 4) * 16, h_dim2)
        ...
        self.fc33 = nn.Linear(h_dim2, z_dim)  # color
        self.fc34 = nn.Linear(h_dim2, z_dim)
        self.fc35 = nn.Linear(h_dim2, z_dim)  # location
        self.fc36 = nn.Linear(h_dim2, z_dim)
        # decoder part
        self.fc4s = nn.Linear(z_dim, h_dim2)  # shape
        self.fc4c = nn.Linear(z_dim, h_dim2)  # color
        self.fc4l = nn.Linear(z_dim, h_dim2)  # location
        self.fc5 = nn.Linear(h_dim2, int(retina_size/4) * int(imgsize/4) * 16) # original
        ...
        self.fc6 = nn.Linear((imgsize * imgsize * 3)+8, imgsize * retina_size * 3)
        #self.fc7 = nn.Linear(28*100*3, 100*28*3)
        self.relu = nn.ReLU()
    ...
    def crop(self, x):
        cur = 0
        while torch.sum(x[:, :, cur]) == 0:
            cur += 1

        crop_start = cur - 3
        crop_end = crop_start + 28
    ...
    '''
        if len(x) == 2:
            img_data = x[0] #.clone()
            pos_list = x[1]
            cropped = img_data[:,:,:,1:29]
            box = torch.full((3, 28, 28), 0.05)
            for i in range(len(img_data)):
                empty_retina = torch.zeros((3, 28, 100))
                crop_start = pos_list[i]
                crop_x = img_data[i][:, :, crop_start:crop_start+28]
                cropped[i] = crop_x 
                empty_retina[0, :, 0:28] = crop_x[0]
                empty_retina[1, :, 0:28] = crop_x[1]
                empty_retina[2, :, 0:28] = crop_x[2]
                
                img_data[i][0, :, crop_start:crop_start+28] = box[0]
                img_data[i][1, :, crop_start:crop_start+28] = box[1]
                img_data[i][2, :, crop_start:crop_start+28] = box[2]
                img_data[i] += empty_retina
            x = img_data
            x = x.cuda()
        else:
            img_data = x[0].clone()
        ...
        '''
    def encoder(self, x):
        # cropping down to the digit within the retina
        #save_image(x[2][0:4], f'{args.dir}/orign.png')
        #x_gray = x[1].clone()
        x = x[3].cuda()
        #save_image(x[0], f'{args.dir}/orig.png')
        h = self.relu(self.bn1(self.conv1(x)))
        hskip = h
        h = self.relu(self.bn2(self.conv2(h)))
        h = self.relu(self.bn3(self.conv3(h)))
        h = self.relu(self.bn4(self.conv4(h)))
        h = h.view(-1, int(retina_size / 4) * int(imgsize / 4) * 16)
        h = self.relu(self.fc_bn2(self.fc2(h)))

        return self.fc31(h), self.fc32(h), self.fc33(h), self.fc34(h), self.fc35(h), self.fc36(h), hskip # mu, log_var
    ...
    def decoder_location(self, z_shape, z_color, h, z_location):
    ...
    def decoder_all(self, z_shape, z_color, z_location, hskip):
        # add location z repr into fc4l
        h = F.relu(self.fc4c(z_color)) + F.relu(self.fc4s(z_shape)) + F.relu(self.fc4s(z_location))
        h = F.relu(self.fc5(h)).view(-1, 16, int(imgsize/4), int(retina_size/4))
        h = self.relu(self.bn5(self.conv5(h)))
        h = self.relu(self.bn6(self.conv6(h)))
        h = self.relu(self.bn7(self.conv7(h)))
        h = self.conv8(h+hskip).view(-1, 3, imgsize, retina_size)
        return torch.sigmoid(h)
    ...
    def forward(self, x, whichdecode='noskip', keepgrad='none'):
        ...
        if(whichdecode == 'all'):
            output = self.decoder_all(z_shape,z_color, z_location, hskip)
        elif (whichdecode == 'noskip'):
            output = self.decoder_noskip(z_shape,z_color, z_location, 0)
        elif (whichdecode == 'skip'):
            output = self.decoder_skip(0, 0, hskip)
        elif (whichdecode == 'color'):
            output = self.decoder_color(0, z_color , 0)
...
#latent space for shape, w/o skip

# build model
def vae_builder(vae_type = vae_type_flag, x_dim = retina_size * imgsize * 3, h_dim1 = 256, h_dim2 = 128, z_dim = 8):
    if vae_type_flag == 'FC':
        vae = VAE_FC(x_dim, h_dim1, h_dim2, z_dim)
    else:
        vae = VAE_CNN(x_dim, h_dim1, h_dim2, z_dim)
...
size1 = retina_size # temporary fix to make adjustments to the loss functions faster

def loss_function_shape(recon_x, x, mu, log_var):
    '''
    img_data = recon_x.clone()
    pos_list = x[1].clone()
    img_crop = img_data[:,:,:,0:28]
    #crop_x=img_data[:,:,:, 1:29] #.cuda()
    for i in range(len(img_data)):
        crop_start = pos_list[i]
        crop_x = img_data[i][:, :, crop_start:crop_start+28]
        crop_start = 36
...
def loss_function_location(recon_x, x, mu, log_var):
    location_recon = recon_x
    #location_recon = recon_x.view(bs, 3, imgsize, retina_size).mean(1)
    #location_recon = torch.stack([location_recon, location_recon, location_recon], dim=1)
    # constructing a 100 by 28 retina with a faint white box marking the location of the digit
    x = x[1].clone()
    x = x.cuda()
    BCE = F.binary_cross_entropy(location_recon.view(-1,retina_size * imgsize * 3), x.view(-1,retina_size * imgsize * 3), reduction='sum')
    KLD = -0.5 * torch.sum(1 + log_var - mu.pow(2) - log_var.exp())
    return BCE + KLD
...
def train(epoch, whichdecode):
    ...
    count=0
    loader=tqdm(train_loader_noSkip) #,total=len(train_loader_noSkip), desc=f"Epoch {epoch+1}/{len(train_loader_noSkip)}")
    for i in loader: #range(1, len(train_loader_noSkip)):
        #print(i)
        if (whichdecode == 'iterated'):
           ...
           if count% 3 == 0:
               ...
               if data_set_flag == 'mnist':
                data1 = dataiter_skip.next()
                data2 = dataiter_skip.next()
                data = data1[0].cuda() + data2[0].cuda()
                #print(data.size())
        ...
        if count % 500 == 0:
            ...
            sample_data[3] = sample_data[3][:sample_size]
            sample = sample_data
            with torch.no_grad():
                reconl, mu_color, log_var_color, mu_shape, log_var_shape,mu_location, log_var_location = vae(sample, 'location') #location
                reconb, mu_color, log_var_color, mu_shape, log_var_shape, mu_location, log_var_location = vae(sample, 'noskip')
            ...
            utils.save_image(
                torch.cat([sample.view(sample_size, 3, imgsize, retina_size), reconb.view(sample_size, 3, imgsize, retina_size), reconl.view(sample_size, 3, imgsize, retina_size),
                           reconc.view(sample_size, 3, imgsize, shape_color_dim), recons.view(sample_size, 3, imgsize, shape_color_dim)], 0),
...
def test(whichdecode):
    ...
    with torch.no_grad():
        for i in range(1, len(test_loader_noSkip)): # get the next batch
            ...
            data = data[0] #.cuda()
            recon, mu_color, log_var_color, mu_shape, log_var_shape, mu_location, log_var_location = vae(data[0].cuda(), 'noskip')
    ...
    data = data[0].cpu()
    data=data.view(bs, 3, imgsize, retina_size)
