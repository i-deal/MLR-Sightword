...
class VAE_CNN(nn.Module):
    def __init__(self, x_dim, h_dim1, h_dim2, z_dim):
        ...
        self.fc4s = nn.Linear(z_dim, h_dim2)  # shape
        self.fc4c = nn.Linear(z_dim, h_dim2)  # color
        self.fc5 = nn.Linear(h_dim2, int(retina_size/4) * int(imgsize/4) * 16)
    ...
    def decoder_noskip(self, z_shape, z_color, hskip):
        #print(z_color.size(),z_shape.size())
        #print(self.fc4c(z_color).size())
        h = F.relu(self.fc4c(z_color)) + F.relu(self.fc4s(z_shape))
        ...
        h = self.conv8(h).view(-1, 3, imgsize, retina_size)
    ...
    def activations(self, z_shape, z_color):
        h = F.relu(self.fc4c(z_color)) + F.relu(self.fc4s(z_shape))
        fc4c = self.fc4c(z_color)
        fc4s = self.fc4c(z_shape)
        fc5 = self.fc5(h)
        return fc4c, fc4s, fc5
    ...
    def forward(self, x, whichdecode='noskip',detachgrad='none'):
...
def loss_function_shape(recon_x, x, mu, log_var):
    ...
    grayrecon = recon_x.view(bs, 3, imgsize, retina_size).mean(1)
    ...
    BCEGray = F.binary_cross_entropy(grayrecon.view(-1, retina_size * imgsize * 3), x.view(-1, retina_size * imgsize * 3), reduction='sum')
...
def loss_function_color(recon_x, x, mu, log_var):
    ...
    recon = recon_x.clone().view(bs, 3, retina_size * imgsize)
    ...
    recon[:, 1, :] = maxg
    recon[:, 2, :] = maxb
    recon = recon.view(-1, retina_size * imgsize * 3)
    ...
    newx[:, 1, :] = maxg
    newx[:, 2, :] = maxb
    newx = newx.view(-1, retina_size * imgsize * 3)
...
def train(epoch, whichdecode):
    ...
    for i in loader: #range(1, len(train_loader_noSkip)):
        ...
        if count % 500 == 0:
            ...
            utils.save_image(
                torch.cat([sample.view(sample_size, 3, imgsize, retina_size), reconskip.view(sample_size, 3, imgsize, retina_size), reconb.view(sample_size, 3, imgsize, retina_size),
